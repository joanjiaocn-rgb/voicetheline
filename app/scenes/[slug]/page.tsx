...
export default async function ScenePage({ params }: ScenePageProps) {
  ...
  return (
    <main className="content-page scene-detail-page">
      <header className="content-header"><a className="wordmark" href="/#studio" aria-label="Voice the Line home"><span className="wordmark-mark">V</span><span className="wordmark-name">VOICE THE LINE</span><b>CUE STUDIO</b></a><a className="content-header-link" href="/voice-over-game/">Voice over game guide <span aria-hidden="true">-&gt;</span></a></header>
      <div className="scene-detail-main">
        <nav className="breadcrumbs" aria-label="Breadcrumb"><a href="/">Voice the Line</a><span aria-hidden="true">/</span><a href="/voice-over-game/">Voice over game</a><span aria-hidden="true">/</span><span>{scene.title}</span></nav>
        <section className="scene-detail-hero"><div className="scene-detail-copy"><span className="section-kicker">Original scene {String(sceneIndex + 1).padStart(2, "0")} / {String(scenes.length).padStart(2, "0")}</span><h1>{scene.title}: voice over game scene</h1><p>{scene.summary}</p><div className="scene-facts"><span>{scene.genre}</span><span>{formatDuration(scene.duration)}</span><span>Timed dialogue</span></div><a className="content-button primary" href={`/?scene=${scene.slug}#studio`}>Play this scene</a></div><div className="scene-detail-media"><Image src={scene.image} alt={`${scene.title} cinematic voice over scene`} fill priority sizes="(max-width: 820px) 100vw, 58vw" style={{ objectPosition: scene.positioning }} /></div></section>
        <section className="scene-detail-content"><article className="scene-note"><span className="section-kicker">Performance note</span><h2>Find the tone before the line</h2><p>{scene.performanceFocus}</p><a className="text-link" href={`/?scene=${scene.slug}#studio`}>Open this scene in the studio <span aria-hidden="true">-&gt;</span></a></article><div className="scene-script"><span className="section-kicker">Timed script</span><h2>Lines to perform</h2><div className="scene-line-list">{scene.lines.map((line) => <div className="scene-line" key={`${line.speaker}-${line.time}`}><time>{formatDuration(line.time)}</time><div><strong>{line.speaker}</strong><p>{line.text}</p></div></div>)}</div></div></section>
        <section className="related-scenes" aria-labelledby="related-scenes-title"><div className="guide-section-heading guide-section-heading-row"><div><span className="section-kicker">Keep exploring</span><h2 id="related-scenes-title">More voice over scenes</h2></div><a className="text-link" href="/voice-over-game/#scene-library">View all 8 scenes <span aria-hidden="true">-&gt;</span></a></div><div className="related-scene-grid">{relatedScenes.map((item) => <a href={`/scenes/${item.slug}/`} key={item.slug}><Image src={item.image} alt={`${item.title} voice over scene`} width={420} height={236} /><span><strong>{item.title}</strong><small>{item.genre}</small></span></a>)}</div></section>
      </div>
