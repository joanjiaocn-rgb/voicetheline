  ...
  return (
    <main className="content-page scene-detail-page">
      ...
      <div className="scene-detail-main">
        ...
        <section className="scene-detail-hero"><div className="scene-detail-copy"><span className="section-kicker">Original scene {String(sceneIndex + 1).padStart(2, "0")} / {String(scenes.length).padStart(2, "0")}</span><h1>{scene.title}: voice over game scene</h1><p>{scene.summary}</p><div className="scene-facts"><span>{scene.genre}</span><span>{formatDuration(scene.duration)}</span><span>Timed dialogue</span></div><a className="content-button primary" href={`/?scene=${scene.slug}#studio`} title={`Play the ${scene.title} scene`}>Play this scene</a></div><div className="scene-detail-media"><Image src={scene.image} alt={`${scene.title} cinematic voice over scene`} fill priority sizes="(max-width: 820px) 100vw, 58vw" style={{ objectPosition: scene.positioning }} /></div></section>
